...
help:
	...
	@echo "  deploy-local-coordinator  Deploy coordinator charm with local resources"
...
.PHONY: deploy-local-coordinator
deploy-local-coordinator:
	@echo "Deploying coordinator charm with local resources..."
	juju deploy $(CHARM_FILE) --resource trino-image=$(REGISTRY)/$(ROCK_NAME):latest --config charm-function=coordinator trino-k8s

.PHONY: deploy-local-worker
deploy-local-worker:
	@echo "Deploying worker charm with local resources..."
	juju deploy $(CHARM_FILE) --resource trino-image=$(REGISTRY)/$(ROCK_NAME):latest --config charm-function=worker trino-k8s-worker
