...
class TestCharm(TestCase):
    ...
    def test_resource_management_config(self):
        ...
        when they are configured, and excludes them when not set.
        """
        harness = self.harness
        simulate_lifecycle_coordinator(harness)

        # Test without resource management config - should not be in environment
        got_plan = harness.get_container_pebble_plan("trino").to_dict()
        environment = got_plan["services"]["trino"]["environment"]

        # Resource management variables should not be present when not configured
        self.assertNotIn("QUERY_MAX_CPU_TIME", environment)
        self.assertNotIn("QUERY_MAX_MEMORY_PER_NODE", environment)
        self.assertNotIn("QUERY_MAX_MEMORY", environment)
        self.assertNotIn("QUERY_MAX_TOTAL_MEMORY", environment)
        self.assertNotIn("MEMORY_HEAP_HEADROOM_PER_NODE", environment)
        ...
        # Check that variables are now present in environment
        got_plan = harness.get_container_pebble_plan("trino").to_dict()
        environment = got_plan["services"]["trino"]["environment"]
