...
variable "channel" {
  description = "Charmhub channel to deploy self-signed-certificates from."
  type        = string
...
variable "offered_endpoints" {
  ...
  validation {
    condition = alltrue([
      for endpoint in var.offered_endpoints : contains(["certificates"], endpoint)
    ])
    error_message = "offered_endpoints may only contain certificates."
  }
}
