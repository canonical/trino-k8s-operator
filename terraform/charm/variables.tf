...
variable "app_name" {
  description = "Application name for the trino-k8s deployment."
  type        = string
  default     = "trino-k8s"
  nullable    = false
...
variable "channel" {
  description = "Charmhub channel to deploy trino-k8s from."
  type        = string
  default     = "latest/edge"
  nullable    = false
}
...
variable "offered_endpoints" {
  ...
  validation {
    condition = alltrue([
      ...
      contains(["grafana-dashboard", "metrics-endpoint", "trino-catalog", "trino-coordinator"], endpoint)
    ])
    error_message = "offered_endpoints may only contain grafana-dashboard, metrics-endpoint, trino-catalog, or trino-coordinator."
  }
}

variable "resources" {
  description = "Charm resource revisions or OCI image overrides, including trino-image. Empty uses resources bundled with the selected charm revision."
  type        = map(string)
  default     = {}
  nullable    = false
}
...
variable "storage_directives" {
  description = "Juju storage directives for the charm's policy storage."
  type        = map(string)
  default     = {}
  nullable    = false
}
